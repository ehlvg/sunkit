...
const ALL_SHAPES: ShapeType[] = [
  ...
  'pentagon', 'hexagon', 'hexagon-flat', 'octagon',
  'star4', 'star5', 'star6',
  'heart', 'parallelogram', 'shield', 'cross',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {([0, 8, 18, 30] as number[]).map((r) => (
        <div key={r} style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
          ...
          {(['square','triangle','diamond','pentagon','hexagon','hexagon-flat','octagon',
            'star5','parallelogram','shield','cross','arrow-right'] as ShapeType[]).map((shape) => (
...
export const WithChildren: Story = {
  ...
  render: (args) => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, alignItems: 'center' }}>
      {(['circle', 'hexagon', 'hexagon-flat', 'square', 'shield', 'star6'] as ShapeType[]).map((shape) => (
  ...
  render: (args) => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, alignItems: 'center' }}>
      {[0, 30, 45, 60, 90, 120, 135, 180].map((deg) => (
        <div key={deg} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
